...
To submit jobs to this cluster, please use the dine2 Slurm queue (or the cosma8-dine2 queue if you have a DiRAC allocation there).  To determine which project you should select, use the `scontrol show partition=dine2` command.  Currently, this includes the durham, do011 (SKA) and do015 projects (use e.g. `#SBATCH -p durham` in your batch submission script).
